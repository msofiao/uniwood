...
export const updateUnreadMessagesCount = async (
  user_id: string,
  ...
  if (!converseExist) throw new Error("Converse document not found");
...
export const converseDocExistsById = async (converes_id: string) => {
...
export const converseDocExistByMessengersId = async (
  messengersId: string[],
) => {
  return await prisma.converse.findFirst({
    where: {
