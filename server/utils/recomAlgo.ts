...
// Interface for a User with relevant preference fields
interface User {
  id: string;
  interests: string[];
...
// Interface for a Post with relevant content fields
interface Post {
  id: string;
  tags: string[];
...
function calculateUserSimilarity(user1: User, user2: User): number {
  ...
  similarityScore += user1.interests.filter((interest) =>
    user2.interests.includes(interest),
  ).length;

  // Check for overlap in liked posts
  similarityScore += user1.liked_posts_id.filter((postId) =>
    user2.liked_posts_id.includes(postId),
  ).length;
...
export async function recommendPosts(
  ...
  try {
    ...
    similarUsers.sort(
      (a, b) =>
        calculateUserSimilarity(user, b) - calculateUserSimilarity(user, a),
    );
    ...
    if (recommendations.length === 0) {
      console.log(
        "No recommendations found using similarity. Using random fallback.",
      );
